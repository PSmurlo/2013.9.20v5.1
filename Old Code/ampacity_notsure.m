...
%Toggles/Inputs
modality=0;             %Landscape:1 Portrait:0
enable_secondary_CB=1;  %0:disable 1:enable
...
for numSpots= floor(400*(min_DC_to_AC)):ceil(400*(max_DC_to_AC));
    ...
    for L=1:numSpots;
        ...
        if(mod(W,1)==0 && isodd(L)==0 && isodd(W)==0)
            ...
            deratedAmpacity=OCPD/(temp_derate*numcond_derate);
            ...
            result_mod=AmpacityVD_results(num_sizes,min_index,VD_percent,price,cond_per_run,runs_mod,wire_l_mod);
            ...
            temp_derate = temp_derate_formula(Ta,tTa,Tc);
            numcond_derate=numcond_derate_chart(cond_per_run);
            deratedAmpacity=Imax/(temp_derate*numcond_derate);
            [min_index] = table310_size(table,type,Tc,deratedAmpacity);
            ...
            for CB_size=CB_size_option
                ...
                if(min_index<30)%((result_mod(1,2)+result_prefab(1,2)+result_trunk(1,2)+VD_offset < VD_desired) || (result_mod(1,2)+result_prefab(1,2)+result_trunk(1,2)-VD_offset < VD_desired))
                    ...
                    result=vertcat(numSpots*ones(1,Leng),L*ones(1,Leng),W*ones(1,Leng),modality*ones(1,Leng),CB_size*ones(1,Leng),num_CB*ones(1,Leng),combinations_length,combinations_runs,combinations_size,combinations_VD,combinations_cost);
