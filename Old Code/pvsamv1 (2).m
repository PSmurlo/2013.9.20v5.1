...
soil(1:12)=0.95;        
% Monthly Soiling Derate
track_mode=0;                   % Tracking mode: 0=fixed, 1=1axis, 2=2axis, 3=azi
dc_derate=0.95;                 % DC Power Derate
num_inverters=5;                 % System size in W
AC_DC_ratio=1.0;                % DC/AC ratio of the system
...
cec_temp_corr_mode=0;           % Cell temperature model , (0=noct, 1=mc)   %not sure which to use
...
stem(y2,hourly_eff,'lineStyle','none')
ylabel('Percentage (%)')
xlabel('Month');
title('Module Efficiency Over a Year');
grid;
