function varargout = guialencon(varargin)
% GUIALENCON MATLAB code for guialencon.fig
%      GUIALENCON, by itself, creates a new GUIALENCON or raises the existing
%      singleton*.
%
%      H = GUIALENCON returns the handle to a new GUIALENCON or the handle to
%      the existing singleton*.
%
%      GUIALENCON('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUIALENCON.M with the given input arguments.
%
%      GUIALENCON('Property','Value',...) creates a new GUIALENCON or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guialencon_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guialencon_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guialencon

% Last Modified by GUIDE v2.5 22-Aug-2013 14:19:33
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @guialencon_OpeningFcn, ...
                   'gui_OutputFcn',  @guialencon_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before guialencon is made visible.
function guialencon_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guialencon (see VARARGIN)
...
global tablelabels
tablelabels = {'Array','Total Cost of System','Max Volt Drop %','Cost of Conductor + Tray',...
...
sortedData = sortrows(finalData);

handles.s1 = scatter(handles.axes,finalData(:,1),finalData(:,2),45,'x');
hold on
grid on
...
i = 1;
j = 1;
handles.sp = scatter(handles.axes,sortedData(i,1),sortedData(j,2),60,'red','o');
...
% Choose default command line output for guialencon
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
% UIWAIT makes guialencon wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guialencon_OutputFcn(hObject, eventdata, handles) 
...
handles = guidata(guialencon);
handles.s1 = s1;
handles.s2 = s2;
handles.s3 = s3;
handles.s4 = s4;
guidata(guialencon,handles);
...
global i j tablelabels finalData sortedData
load('sizes.mat')
...
set(handles.sp,'xdata',sortedData(j,1));
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global i j sortedData tablelabels
load('sizes.mat')
