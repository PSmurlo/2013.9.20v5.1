function output_txt = PVDatatipCursorKWH(obj,event_obj)
...
output_txt = {['25-yr kWh loss: ',num2str(pos(1),4)],...
              ['Max VD Percent: ',num2str(pos(2),4)]};
...
if sum(dataSize>1)>1
    ...
    output_txt{end+1} = ['Index (', subsString(1:end-1), ')'];
else
    % display linear index for vector data
    output_txt{end+1} = ['Index: ',num2str(dataIndex)];
end