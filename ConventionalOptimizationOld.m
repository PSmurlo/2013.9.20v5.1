function [finalData] = ConventionalOptimization(handles)
...
%         if(info.qH/(info.numCB*2/inv.num_inverters) >= mod(info.qH,(info.numCB*2/inv.num_inverters)))
            ...
            % info.qW==2 && info.qH==11 && info.numCB==110 && info.index==13
            [raw.wire.m2cb,longestm2cb1way] = stringRuns(info.qW,info.qH,eleDim.totW,eleDim.totH,info.numCB/(2*(inv.num_inverters/4)),eleDim.modality,info.CBqH);
            ...
            [cost,raw] = ItemtoCostConversionConv(raw,trunkInfo(:,8));
